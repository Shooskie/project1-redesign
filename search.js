...
function search() {
    ...
    $.each(movies, function (i, val) {
        var start = movies[i].movies.title.toLowerCase().search(value.toLowerCase().trim());//Supposed to iterate through the array of movies to find if there is a match
        if (start != -1) { //if there is a search match
            html += "<div class='sub_suggestions' data-item='" + movies[i] + "' >";
            html += "<b>" + movies[i].title + "</b> (" + movies[i].year + "), Starring: " + movies[i].starring;
            html += "</div>";
